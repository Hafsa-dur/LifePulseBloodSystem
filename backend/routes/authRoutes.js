...
router.post('/rewards/send', requireAuth, async (req, res) => {
	try {
		...
		const recipientEmail = String(donorEmail || req.user.email || '').trim().toLowerCase();
		...
		const result = await sendRewardEmail({ donorEmail: donor.email, donorName: donor.donorName || req.user.name, rewardTitle, partner, points, voucherCode });
