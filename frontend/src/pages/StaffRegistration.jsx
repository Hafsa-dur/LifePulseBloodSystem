...
const StaffRegistration = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!token || !validation.valid) return setStatus({ type: 'error', text: 'A valid staff invitation link is required.' });
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#FAF9F6] flex items-center justify-center py-12 px-4 font-sans">
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-2xl border-2 border-[#5A1827]/20">
        <div className="text-center mb-7">
          ...
          <p className="mt-2 text-xs font-bold text-slate-500">{validation.hospitalName ? `Invitation for ${validation.hospitalName}` : 'Complete this form using the invitation provided by your Hospital Admin.'}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {['name', 'email', 'phone'].map((field) => (
            <label key={field} className="block text-xs font-black uppercase tracking-wider text-[#5A1827]">
              ...
              <input name={field} type={field === 'email' ? 'email' : 'text'} required={field !== 'phone'} readOnly={field === 'email' && Boolean(validation.email)} value={form[field]} onChange={updateField} className="mt-1 w-full rounded-xl border-2 border-[#5A1827]/20 bg-[#FAF9F6] p-3 text-sm normal-case tracking-normal focus:outline-none focus:border-[#5A1827]" />
            </label>
          ))}
